...
public:
    vector<vector<string>> solveNQueens(int n) {
        ...
        vector<string> board;
        int i_start = 0;
        solveNQueens_helper(answer, n, board, i_start);
        return answer;
    }

    // n: chessboard width, q_num: existing queens number, board_tmp: working chessboard
    bool solveNQueens_helper(vector<vector<string>> &answer, int n, vector<string> &board_tmp,
                             int i_start)
    {
        for (int j = 0; j < n; ++j)
        {
            ...
            if (q_valid(board_tmp, n, i_start, j))
            {
                ...
                if (i_start == n-1)
                {
                    answer.emplace_back(board_tmp);
                    board_tmp[i_start][j] = '.';
                    return true;
                }
                else
                {
                    solveNQueens_helper(answer, n, board_tmp, i_start+1);
                    board_tmp[i_start][j] = '.';
                }
            }
        } return false;
